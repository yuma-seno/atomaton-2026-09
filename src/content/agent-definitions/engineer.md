...
- The `files` server's tools carry no prefix: `read`, `grep`, `glob`, `edit`, `write`, `list`. There is no tool that returns the whole tree, and no question whose answer is one. Use `list` for a directory, or `glob` to find a path by name — it matches paths against a pattern and never reads a file, so it finds *where* something is, not *what* contains a string.
