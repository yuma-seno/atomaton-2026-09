...
Read **all** of `.github/atomaton/agent-definitions/*.md`, not just the one in the
diff.

Every name under an agent's `mcp_servers` has to resolve to a server the run will
start, and two things provide one: the servers Atomaton ships, which are **not** in
...
- An entry whose name is one Atomaton ships is an **override**, merged field by
