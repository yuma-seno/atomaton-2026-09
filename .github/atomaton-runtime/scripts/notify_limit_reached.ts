...
// src/entrypoints/machinery/notify_limit_reached.ts
...
// src/domain/record/tool-tally.ts
var NAMED = 4;
...
function main() {
  ...
  const notice = values.notify ? `@${values.notify} Atomaton: \`${values.agent}\` ran out of time. Review the issue and comment \`/${values.agent}\` to retry.` : `Atomaton: \`${values.agent}\` ran out of time. Comment \`/${values.agent}\` to retry.`;
